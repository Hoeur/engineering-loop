  ...
  describe('prompt-injection scan', () => {
    const setup = (context: Record<string, unknown>) => {
      ...
      const executor = new AgentExecutor({
        ...
        credentials: { resolve: vi.fn().mockResolvedValue(undefined) },
        credentialStore: { set: setCredential, clear: vi.fn(), env: vi.fn().mockReturnValue({}) },
        contextBuilder: {
          resolveRoleProvider: vi.fn().mockResolvedValue({
      ...
      return { executor, startRun, updateMany, recordAudit, setCredential };
    };
    ...
    it('refuses to prime credentials or spawn the provider on a HIGH finding', async () => {
      const { executor, startRun, updateMany, recordAudit, setCredential } = setup({
